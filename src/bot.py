...
async def apply(ctx, email, year_raw, rules):
    ...
    if rules != "yes":
        ...
        await deletemsg(ctx)
        logger.info("Application Denied - Reason: Rules - Applicant: {0}".format(ctx.message.author.name))
    ...
    if len(year_raw) > 4 and year_raw[0] != "2" and year_raw[1] != "0" and (year_raw[2] != "2" or year_raw[2] != "1"):
        await ctx.message.author.send("{0}! You must provide a valid grad year! "
                                      ...
                                      .format(ctx.message.author.mention))
        await deletemsg(ctx)
        logger.info("Application Denied - Reason: Invalid Grad Year - Applicant: {0}".format(ctx.message.author.name))
        print(ctx.message.content)
        return
    ...
    if email.split("@")[1] != "ontariotechu.net":
        await ctx.message.author.send("{0}! You must provide a valid OntarioTechu.net email! "
                                      ...
                                      .format(ctx.message.author.mention))
        await deletemsg(ctx)
        logger.info("Application Denied - Reason: Invalid OTU Email - Applicant: {0}".format(ctx.message.author.name))
        print(ctx.message.content)
        return
